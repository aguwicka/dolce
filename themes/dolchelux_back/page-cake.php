...
<?php get_template_part('form_template/tastingToppings');?>
    <script>
        $(document).ready(function () {
            $('#cakes_orders_phone ').inputmask({"mask": "+375 (99) 999-99-99"});
            $('.cakes-orders__upload-file').on('click',function(){
                $('#cakes_orders_cake_photo').trigger('click');
            })
            $('#cakes_orders_cake_photo').on('change', function(){
                let _this = $(this);
                ...
                if (_this[0].files && _this[0].files[0]) {
                    ...
                    $('#file-name').text(_this[0].files[0].name);

                    reader.readAsDataURL(_this[0].files[0]);

                    $('.cakes-orders__preload-image').removeClass('d-none');
                    $('#file-name').removeClass('d-none');
                } else {
                    $('.cakes-orders__preload-image').addClass('d-none');
                    $('#file-name').addClass('d-none');
                }
            })
            $('#cakes_orders_from_gallery').iCheck({
                    ...
                    <figure class="w-75" onclick="return true">
                        ...
                        <div class="cakes__cakes-figcaption-wrapper">
                            <figcaption class="cakes__cakes-caption">
                                ...
                                <p class="mt-3"><a class="cakes__cakes-show-link" data-toggle="modal" data-target="#modal-<?php echo get_the_ID($recent)?>">ПОСМОТРЕТЬ</a></p>
                            </figcaption>
                        </div>
                    </figure>
                </div>
